...
export function filterTasksBySearch(tasks, searchQuery, activeSearchMode) {
   ...
   return tasks.filter((task) => {
      switch (activeSearchMode) {
         ...
         case 2:
            return Array.isArray(task.subtasks) && task.subtasks.some((subtask) => subtask.title?.toLowerCase().includes(normalizedSearch));
         case 3:
            return Array.isArray(task.categories) && task.categories.some((category) => String(category).toLowerCase().includes(normalizedSearch));
         case 4:
...
function getStartOfWeek(date) {
   ...
   return new Date(startOfDay.getFullYear(), startOfDay.getMonth(), startOfDay.getDate() + mondayShift);
}
...
export function filterTasksByOptions(tasks = [], activeFilters = []) {
   ...
   const endOfCurrentWeek = new Date(startOfCurrentWeek.getFullYear(), startOfCurrentWeek.getMonth(), startOfCurrentWeek.getDate() + 6);
   const startOfNextWeek = new Date(endOfCurrentWeek.getFullYear(), endOfCurrentWeek.getMonth(), endOfCurrentWeek.getDate() + 1);
   ...
   return tasks.filter((task) => {
      ...
      const isToday = Boolean(taskDate && getStartOfDay(taskDate).getTime() === startOfToday.getTime());
      const isTomorrow = Boolean(taskDate && getStartOfDay(taskDate).getTime() === startOfTomorrow.getTime());
      const isThisWeek = Boolean(taskDate && taskDate >= startOfCurrentWeek && taskDate <= endOfCurrentWeek);
      const isNextWeek = Boolean(taskDate && taskDate >= startOfNextWeek && taskDate <= endOfNextWeek);
...
export function sortTasksByDate(tasks, now) {
