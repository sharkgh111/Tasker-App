...
import Toggle from "@/Components/UI/TaskToggle";
import FieldInput from "@/Components/UI/TaskInput";

export default function TaskManager({ data, setData, errors, clearErrors, task, onToggleChange }) {
      ...
      <section className="flex flex-col flex-1  min-w-0 min-h-0 px-3 py-5 w-[calc(var(--overall-w)-2px)] h-full overflow-y-auto flex-shrink-0 border-r-[3px] border-main_lightly/30">
         ...
         <div className="flex flex-col flex-1 items-start py-5 gap-[40px] justify-center">
            <div className="w-full flex items-center flex-col justify-start gap-2 border-b-4 pb-3 border-main_lightly/30">
               ...
               />
               {errors.title && <span className="text-danger_light text-md font-montserrat-medium self-start">{errors.title}</span>}
            </div>
            <div className="w-full flex flex-col items-center justify-start gap-2 border-b-4 pb-3 border-main_lightly/30">
               ...
               />
               {errors.description && <span className="text-danger_light text-md font-montserrat-medium self-start">{errors.description}</span>}
            </div>
            <div className="w-full flex flex-col items-center justify-start gap-2 border-b-4 pb-3 border-main_lightly/30">
               <FieldInput
                  label="Термін"
               ...
               />
               {errors.taskDate && <span className="text-danger_light text-md font-montserrat-medium self-start">{errors.taskDate}</span>}
            </div>
            {data.is_planned && (
               <div className="w-full flex flex-col items-center justify-start gap-2 border-b-4 pb-3 border-main_lightly/30">
                  <FieldInput
                     ...
                     className="cursor-pointer select-none [color-scheme:dark]"
                  />
                  {errors.uploadDate && <span className="text-danger_light text-md font-montserrat-medium self-start">{errors.uploadDate}</span>}
               </div>
            )}
         </div>
      </section>
