...
export default function Task({ tasks, openModal, isPlanned = false }) {
    ...
    const toggleSubtask = (id) => {
        setData('subtasks', data.subtasks.map(sub => 
                        ...
                        <section key={task.id} className="flex flex-col items-center shadow-xl justify-between w-full bg-main_green_light border-2 border-main_lightly rounded-xl transition-all">
                                ...
                                <div className={`flex flex-1 ${isPlanned ? 'px-2' : 'px-4'} py-4 flex-row gap-4 items-center justify-between`}>
                                    <div className={`flex flex-1 ${isPlanned ? 'flex-row items-center' : 'flex-col items-start'} justify-between`}>
                                        <div className="flex flex-col items-start text-base justify-center">
                                            <h3 className={`text-2xl font-montserrat-medium transition-all duration-300 
                                    ...
                                    <div className="flex flex-col-reverse gap-2 items-end justify-between h-full">
                                        ...
                                        {!isPlanned && (
                                            <span className={`text-[12px] uppercase tracking-wider font-montserrat-bold px-2 py-0.5 border-2 shadow-lg rounded-md whitespace-nowrap transition-all duration-300 ${taskStatus.color}`}>
                                                {taskStatus.text}
                                            </span>
                                        )}
                                        ...
                                        {currentPriority && (
                                            <span className={`w-5 h-5 rounded-full border-2 border-main_lightly bg-${currentPriority.color}`}>
                                            </span>
                                        )}
                                        </div>
                                        {task.categories && task.categories.length > 0 && (
                                            <div className="flex flex-wrap items-center justify-end gap-1">
                                                {task.categories.map((categoryName, idx) => {
                                                    const originalCategory = TASK_CATEGORIES.find(c => c.name === categoryName);
                            ...
                            <div className={`flex w-full items-center justify-between gap-4 ${isPlanned ? 'px-2' : 'px-5'}`}>
                                <div className="flex-none w-1/5 min-w-0 py-3"> 
                                    {task.description && (
                                        <p className={`font-montserrat-medium break-words whitespace-pre-wrap text-left w-full text-lg transition-all duration-300 
