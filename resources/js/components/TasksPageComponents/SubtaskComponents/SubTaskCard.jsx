...
export default function SubTaskCard({ subtask, isTaskActive, taskStatus }) {
     ...
     return (
          <div className="w-full [counter-increment:subtask-counter] flex flex-1 items-center text-center justify-between rounded-lg px-2 py-3 bg-main_green_dark/20">
               <span className="text-main_lightly font-montserrat-bold before:content-[counter(subtask-counter)]"></span>
               <span
                    className={`font-montserrat-bold text-base text-center ${taskStatus.text === "Протерміновано" ? " text-main_lightly/50" : "text-main_lightly"} ${subtask.is_completed ? "line-through text-main_lightly/50" : "text-main_lightly"}`}
               >
                    {subtask.title}
               </span>
               {isTaskActive && (
                              ...
                              <button
                                   ...
                                   className={`w-7 h-7 rounded-lg border-2 flex items-center justify-center transition-all duration-200 ${
                                        subtask.is_completed
                                             ? "border-succes_light text-succes_light shadow-lg"
