     ...
     return (
          <Popover className={popoverClass}>
               <PopoverButton className="outline-none">
                    <Icon
                         className={`text-main_lightly transition-all duration-300 hover:text-main_lightly/80 hover:scale-95 cursor-pointer ${iconSize}`}
                    />
               </PopoverButton>
               <PopoverPanel
                    anchor={{ to: "bottom end", offset: "20px" }}
                    className="flex flex-col bg-main_green_dark/40 backdrop-blur-sm border border-main_lightly/30 rounded-xl shadow-xl mt-3"
               >
                    {({ close }) => (
                         <>
                              {type === "search" && (
                              ...
                              {type === "filter" && (
                                   <div className="flex gap-0 flex-col text-main_lightly">
                                        <p className="font-montserrat-medium text-center text-xl text-main_lightly border-b-2 py-2 px-6 border-main_lightly/20">
                                             Фільтрація задач
                                        </p>
                                        <div className="grid grid-cols-2 w-full text-base font-montserrat-regular">
                                             {array.map((item) => {
                                                  const isSelected =
                                                       Array.isArray(tempFilters) &&
                                        ...
                                        <div className="w-full flex items-center justify-between border-t-2 py-3 px-6 border-main_lightly/20">
                                             ...
                                             <Button
                                                  ...
                                                  onClick={() => {
                                                       ...
                                                       if (
                                                            typeof handleApplyFilters === "function"
                                                       ) {
