...
export default function FieldInput({ label, type = "text", placeholder, Icon, className = "", ...props }) {
     ...
     return (
          <div className={`flex flex-col gap-2 w-full ${type === "textarea" ? "h-full" : ""}`}>
               ...
               {type === "textarea" ? (
                    <textarea placeholder={placeholder} className={`${baseStyles} resize-none flex-1 ${className}`} {...props} />
               ) : (
                    <input type={type} placeholder={placeholder} className={`${baseStyles} ${className}`} {...props} />
               )}
          </div>
     );
}
