...
export default function SubTaskManager({ subtasks = [], setData, isAddingSubtask, setIsAddingSubtask, setNewSubtaskTitle, newSubtaskTitle }) {
          ...
          >
               {subtasks.length > 0 && (
                    <div className="w-full flex items-center justify-center gap-2 border-b-4 pb-2 border-main_lightly/30">
                         ...
                         <h3 className="text-xl font-montserrat-bold text-main_lightly">Підзавдання</h3>
                    </div>
               )}
               {isAddingSubtask && (
               ...
               {subtasks.length === 0 && !isAddingSubtask && (
                    <div className="flex flex-col items-center gap-3">
                         <p className="text-main_lightly font-montserrat-medium text-md text-center">У цієї задачі ще немає підзадач</p>
                         <Button
                              type="button"
               ...
               {subtasks.length > 0 && (
                    <div className="flex flex-col gap-3 w-full pr-1">
                         {subtasks.map((sub) => (
                              <div key={sub.id} className="flex items-center justify-between bg-main_green_dark/50 border-2 border-main_lightly p-3 rounded-xl shadow-md transition-all animate-fadeIn">
                                   <div className="flex items-center gap-4">
                                        <span className={`text-md font-montserrat-medium break-words transition-all ${sub.is_completed ? "line-through text-main_lightly/40" : "text-main_lightly"}`}>
                                             {sub.title}
                                        </span>
                                   </div>
