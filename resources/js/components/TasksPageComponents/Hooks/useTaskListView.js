...
import {
     filterTasksByOptions,
     filterTasksBySearch,
     isTaskPlanned,
     parseTaskUploadDate,
     sortTasksByDate,
     sortTasksByStatusAndDate,
...
export function useTaskListView(tasks = [], activeFilters = []) {
     ...
     const currentRaw = useMemo(
          () =>
               currentFilteredTasks.filter((task) => {
                    ...
                    return (
                         !isTaskPlanned(task.is_planned) ||
                         uploadDateMs <= now.getTime() ||
                         Number.isNaN(uploadDateMs)
                    );
               }),
     ...
     const currentTasks = useMemo(
          () => sortTasksByStatusAndDate([...currentFuture, ...currentOverdue], now),
          [currentFuture, currentOverdue, now],
     );
